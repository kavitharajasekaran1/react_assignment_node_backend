...
const addTransaction = async (req, res) => {
  ...
  finalObj = { name: name, date: date, amount: amount, rewardPoints: reward };
...
const getAllTransactions = async (req, res) => {
  try {
    ...
    if (transactions != undefined && transactions.length > 0) {
      ...
      var secondDataResult = [
        {
          month: monthNames[d.getMonth()],
          amount: firstMonth.reduce(function (sum, current) {
            return sum + Number(current.amount);
          }, 0),
          rewardPoints: firstMonth.reduce(function (sum, current) {
            return sum + Number(current.rewardPoints);
          }, 0),
        },
        {
          month: monthNames[d.getMonth() + 1],
          amount: secondMonth.reduce(function (sum, current) {
            return sum + Number(current.amount);
          }, 0),
          rewardPoints: secondMonth.reduce(function (sum, current) {
            return sum + Number(current.rewardPoints);
          }, 0),
        },
        {
          month: monthNames[d.getMonth() + 2],
          amount: thirdMonth.reduce(function (sum, current) {
            return sum + Number(current.amount);
          }, 0),
          rewardPoints: thirdMonth.reduce(function (sum, current) {
            return sum + Number(current.rewardPoints);
          }, 0),
        },
      ];
